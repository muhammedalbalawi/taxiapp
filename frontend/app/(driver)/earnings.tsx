    ...
    <SafeAreaView style={[styles.container, { backgroundColor: colors.bg }]}>
      <ScrollView contentContainerStyle={{ padding: 24 }}>
        ...
        <View style={[styles.hero, { backgroundColor: colors.primary }]}>
          ...
          <Text style={styles.heroValue}>${data.today_earnings.toFixed(2)}</Text>
          <Text style={styles.heroSub}>{data.today_trips} {t('trips_count').toLowerCase()}</Text>
        </View>
        <View style={styles.grid}>
          <Stat label={t('total_earnings')} value={`$${data.total_earnings.toFixed(2)}`} icon="wallet" />
          <Stat label={t('trips_count')} value={data.total_trips} icon="car" />
        </View>
      </ScrollView>
    </SafeAreaView>
  );
...
const styles = StyleSheet.create({
  ...
  heroValue: { color: '#fff', fontSize: 48, fontWeight: '900', marginTop: 8, letterSpacing: -1 },

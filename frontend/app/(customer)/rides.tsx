...
export default function Rides() {
  ...
  const load = useCallback(async () => {
    try { setTrips(await api('/api/rides/mine', { token: sessionToken })); } catch {}
  }, [sessionToken]);
          ...
          <View style={[styles.card, { backgroundColor: colors.surface, borderColor: colors.border }]}>
            <View style={styles.cardTop}>
              <Text style={[styles.cardType, { color: colors.primary }]}>
                {t(item.ride_type)} • {t(item.status) || item.status}
              </Text>
              <Text style={[styles.cardPrice, { color: colors.textPrimary }]}>{formatPrice(item.price, lang)}</Text>
            </View>
            ...
            <Text style={[styles.date, { color: colors.textSecondary }]}>
              {item.distance_km} {t('km')} • {item.duration_min} {t('min')} • {item.payment_method || 'cash'}
            </Text>
          </View>
        )}
      />

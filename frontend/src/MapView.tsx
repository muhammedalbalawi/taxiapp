...
const buildHTML = (center: {lat:number;lng:number}, markers: Marker[], zoom: number, dark: boolean) => {
  const tile = dark
...
<style>
  ...
  @keyframes pulse {
    ...
    70% { box-shadow:0 0 0 26px rgba(0,102,255,0); }
...
export default function MapView({ center, markers = [], zoom = 13, style }: Props) {
  const { mode } = useApp();
  const html = buildHTML(center, markers, zoom, mode === 'dark');
